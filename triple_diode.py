...
class TripleDiode:
    """
    Triple-diode solar-cell parameter estimation problem.
    ...
    Isd1 : First diode saturation current
    Isd2 : Second diode saturation current
    Isd3 : Third diode saturation current
    n1   : First diode ideality factor
    n2   : Second diode ideality factor
    n3   : Third diode ideality factor
    """
    ...
    def predict_current(self, solution) -> np.ndarray:
        ...
        if (
            ...
            raise ValueError(
                "Rsh, n1, n2 and n3 must be greater than zero"
            )
        ...
        diode_current1 = isd1 * (
            np.exp(exponent1) - 1.0
        )

        diode_current2 = isd2 * (
            np.exp(exponent2) - 1.0
        )

        diode_current3 = isd3 * (
            np.exp(exponent3) - 1.0
        )
